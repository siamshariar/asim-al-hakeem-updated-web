...
export const getQnaByLimit = async (num) => {
	if (num && num > 0) {
		return loadQnaData().qna.slice(0, num).map(normalizeQna).filter(Boolean);
	} else return loadQnaData().qna.slice(0, constants.GENERATED_ANS_PAGE).map(normalizeQna).filter(Boolean);
};
...
export const getAnsById = async (id) => {
	return loadQnaData().qna.filter((item) => item.id == id).map(normalizeQna).filter(Boolean);
};
