...
const normalizeQna = (item) => {
	...
	const categories = item.categories || (item.cat_slug ? [item.cat_slug] : []);
	...
	return {
		...
		cat_slug: item.cat_slug || categories[0] || "",
	};
};
...
export const getHomeQna = async () => {
	const items = qna.filter((item) => item.home === "1" || item.featured === true);
	return items.map(normalizeQna).filter(Boolean);
};
...
export const getAllQuestions = async (obj) => {
	...
	if (cat_slug == "all") {
		dataSet = qna.map(normalizeQna).filter(Boolean);
	} else {
		dataSet = qna.filter((item) => item.cat_slug == cat_slug).map(normalizeQna).filter(Boolean);
	}
